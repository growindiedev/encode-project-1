...
describe("Ballot", function () {
  ...
  describe("when the voter interact with the vote function in the contract", function () {
    ...
    it("proposal vote count increments by voter's vote weight on successful vote", async function () {
      ...
      await ballotContract.connect(accounts[1]).vote(0);
      expect(ballotContract.proposals[0].voteCount.toNumber()).to.eq(
        voterWeight
      );
    });
  ...
  describe("when someone interact with the winningProposal function before any votes are cast", function () {
    ...
    it("if no votes are cast yet, it'll output the last proposal", async function () {
      const winningProposal = await ballotContract
        .connect(accounts[1])
        .winningProposal();
      expect(winningProposal).to.eq(
        ballotContract.proposals[ballotContract.proposals.length - 1]
      );
    });
  });
  ...
  describe("when someone interact with the winnerName function before any votes are cast", function () {
    ...
    it("if no votes are cast yet, it'll output the name of the last proposal", async function () {
      const winningName = await ballotContract
        .connect(accounts[1])
        .winningName()
        .toString();
      expect(winningName).to.eq(
        ballotContract.proposals[
          ballotContract.proposals.length - 1
        ].name.toString()
      );
    });
  });
